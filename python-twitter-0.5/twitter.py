...
class Api(object):
  ...
  def SocialGraphGetFollowers(self, user=None):
    '''Fetch a list of follower ids for a user'''
    if user == None:
      print "You must provide a username or id"
    else:
      url = 'http://twitter.com/followers/ids/%s.json' % user
      json = self._FetchUrl(url)
      data = simplejson.loads(json)
      return data

  def SocialGraphGetFriends(self, user=None):
    '''Fetch a list of friend ids for a user'''
    if user == None:
      print "You must provide a username or id"
    else:
